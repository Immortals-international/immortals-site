...
function renderLayout(){const p=layouts[layoutIndex];replacePlanImage('#plan-image',p,`${p.name} floor plan: ${p.title}`);document.querySelector('#open-plan').setAttribute('aria-label',`Enlarge the ${p.name} floor plan`);document.querySelector('#plan-title').textContent=p.title;document.querySelector('#plan-description').textContent=p.description;document.querySelector('#plan-tradeoff').textContent=p.tradeoff;document.querySelector('#allocation').innerHTML=p.areas.map((a,i)=>`<div class="allocation-row"><div class="allocation-label"><span>${['Gallery','Lounge + coffee bar','Baccarat / flexible salon'][i]}</span><span>${a} m²</span></div><div class="allocation-bar"><div class="allocation-fill" style="width:${a/168*100}%"></div></div></div>`).join('');document.querySelectorAll('[data-layout]').forEach(b=>{const a=Number(b.dataset.layout)===layoutIndex;b.classList.toggle('active',a);b.setAttribute('aria-pressed',a)});if(planDialog.open)renderLargePlan()}
